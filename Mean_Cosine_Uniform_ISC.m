...
for i=1:size(Attractor_Recall)
    ...
    for t=1:size(Attractor_Recall{i}{j,5},1)
...
%     M_2=abs(M-0.5)/0.5;
%     Attractor_Recall{i}{j,10}=M;
%     Attractor_Recall{i}{j,11}=mean(M);
%     
%     Attractor_Recall{i}{j,12}=M_2;
%     Attractor_Recall{i}{j,13}=mean(M_2);
    Attractor_Recall{i}{j,8}=M;
    Attractor_Recall{i}{j,9}=mean(M);
    clear M %M1 M2 M M_2
